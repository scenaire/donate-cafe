...
npm run backfill:assets -- --dry-run   # Re-encode existing bucket assets to WebP
...
- **`/` ([app/page.tsx](app/page.tsx))** — Tip form. Submits to `/api/create-payment-intent`, then polls `/api/check-status` every 3s while showing a PromptPay QR with a 10-minute countdown. Card payments use the Stripe Payment Element in the deferred flow (intent created only at Pay time). Opens behind a **boot curtain** (`CafeCurtain`) that holds the plum ground until the viewport class, the pixel fonts, `/api/menu` + `/api/cafe-config` + `/api/goal`, and the two bitmaps this load will actually paint are all resolved — every wait is capped (`BOOT_MAX_MS`), so a slow asset delays nothing.
...
- **`/live` ([app/live/page.tsx](app/live/page.tsx))** — Glanceable monitoring feed for while you're streaming. Subscribes to a second Supabase Broadcast channel (`live-$live_token`) and reconciles every 30s against `/api/live/feed`. Unlike `/alert` it shows held, hidden, pending and failed orders — it's the creator's own view. **EXPIRED is excluded** (in the query *and* the client merge): an expired QR is someone who never paid, and it's the most common row in the table. `/dashboard` remains the full audit trail. Requires the same admin session as `/dashboard`.
...
| `GET /api/live/feed` | Session-gated. Backs `/live`: returns the channel token **and** the 50 most recent orders in one round trip. The token is returned in the body and never in a URL. |
...
- **[lib/live.ts](lib/live.ts)** — **SERVER ONLY.** `publishLiveUpdate()` for the `/live` feed. The deliberate inverse of `publishAlert`: no `show_on_screen` or moderation gate, because that view exists to show what the overlay refuses.
- **[lib/live.shared.ts](lib/live.shared.ts)** — Channel contract for `/live`, shared with the browser. Same no-server-imports rule as `realtime.shared.ts`.
...
- **[lib/image.ts](lib/image.ts)** — **SERVER ONLY** (native `sharp`). `toWebp()` — the one place asset bytes are normalised, shared by the upload route and the asset backfill so new and converted files come out identical. The tip page renders these straight from the bucket with no `next/image`, so there is no later optimisation step.
...
- **`show_on_screen`**: set per order; hidden tips never reach the overlay (enforced in `publishAlert` *and* in the alerts query). They *do* reach `/live` — see below.
- **Two Realtime channels, two tokens.** `alerts-$widget_token` (overlay) carries only approved, on-screen tips; its token lives in an OBS URL and is therefore semi-public. `live-$live_token` (the `/live` feed) carries every order including held message text, so it must NOT reuse the widget token — anyone with the browser-source URL would then be able to read held and hidden messages. `live_token` is only ever served through a `requireAdmin()`-gated route body, which keeps the admin session as the real authorisation boundary.
